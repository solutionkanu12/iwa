...
import { CeloCollectService } from "./collect";
...
describe("CeloCollectService", () => {
  it("sends collect() to the bound circle contract, tagged exactly once", async () => {
    const provider = mockProvider();
    const service = new CeloCollectService({ circleContract: CIRCLE_CONTRACT }, provider);
    const result = await service.collect(CALLER);
    ...
    expect(tx.data.startsWith(encodeCollect())).toBe(true);
  ...
  it("collect() takes no destination argument: the wire calldata is the bare selector", () => {
    // encodeCollect() has no parameters at all — there is no `to` to inject.
    expect(encodeCollect()).toMatch(/^0x[0-9a-f]{8}$/);
  });

  it("rejects on the wrong network before sending anything", async () => {
    const provider = mockProvider({ chainId: "0xaa36a7" });
    const service = new CeloCollectService({ circleContract: CIRCLE_CONTRACT }, provider);
    await expect(service.collect(CALLER)).rejects.toThrow(/Celo mainnet/);
    expect(provider.sends).toHaveLength(0);
  });

  it("does not mark collect complete when the wallet rejects", async () => {
    const provider = mockProvider({ sendError: new Error("user rejected") });
    const service = new CeloCollectService({ circleContract: CIRCLE_CONTRACT }, provider);
    await expect(service.collect(CALLER)).rejects.toThrow(/rejected/);
  });

  it("reports FAILED, not CONFIRMED, for a reverted transaction", async () => {
  ...
  it("refuses an untagged raw send through the adapter provider it wraps", async () => {
    const provider = mockProvider();
    const service = new CeloCollectService({ circleContract: CIRCLE_CONTRACT }, provider);
    await service.collect(CALLER);
    // The service wraps the raw provider; the wrapped provider must still
    // refuse eth_sendRawTransaction the same way every other Celo path does.
  ...
  it("normalizes the circle contract address at construction, refusing a malformed one", () => {
    expect(() => new CeloCollectService({ circleContract: "not-an-address" }, mockProvider())).toThrow();
  });
});
