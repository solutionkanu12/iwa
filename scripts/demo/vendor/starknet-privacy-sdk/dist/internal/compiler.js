...
export class ActionCompiler {
    ...
    transformToClientActions(actions, pool, recipientsNeeded, options) {
        ...
        if (actions.computeAndInvoke) {
            ...
            const compute_additional_data = CallData.compile(details.computeAdditionalData ?? []).map(toBigInt);
            const invoke_additional_data = CallData.compile(details.invokeAdditionalData ?? []).map(toBigInt);
            const input = {
    ...
    async resolveRecipientChannels(actions, options, registry, recipientsNeeded) {
        ...
        const resolvedTotal = total ??
            (await this.discoveryProvider.discoverChannels(this.userAddress, this.userViewingKey, "total-only", { blockIdentifier: options?.provingBlockId })).total;
